    ...
    return (
        <div>
            <div style={{ bottom: 130, right: 8, position: 'absolute', zIndex: 2, cursor: 'pointer' }}>
                <button disabled={value <= capabilities.min} onClick={handleZoomOut}>Zoom out</button>
            </div>
            <div style={{ bottom: 180, right: 8, position: 'absolute', zIndex: 2, cursor: 'pointer' }}>
                <button disabled={value >= capabilities.max} onClick={handleZoomIn}>Zoom in</button>
            </div>
        </div>
    );
}